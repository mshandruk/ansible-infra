...
> Note: By default, the port for Ansible is allowed in the `filter` table's `input` chain

## Features

- Install iptables, iptables-persistent.
- Enables IPv4 forwarding in sysctl.
- Disallow IPv6 default policy(except  OUTPUT).
- Configure source NAT (masquerade).
- Manages custom INPUT, FORWARD, and DNAT rules.
- Reverting rules if management access has been blocked.
